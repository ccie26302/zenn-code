...
```bash
.venv/bin/python run.py acc      # 精度・確率の質（並列。注: SDK のリトライは実際には無効。下記）
...
.venv/bin/python run.py embtrain # 分類器の学習用埋め込みを保存（report.py の交差検証用）
.venv/bin/python run.py think    # 追加測定: 2.5 Flash に思考を与えた条件（精度→速度）
```
...
.venv/bin/python report.py           # 記事の表をすべて出す（主分析と再試行込み、別の言い回し、コスト、予測の判定）
```

## 事前登録

**計画と実装の食い違い**: `PLAN.md` では「精度側は SDK のリトライ有効」としていますが、`run.py` の `client_retry()` は `retry_options` を渡しておらず、google-genai 2.24.0 の既定ではリトライしません。そのため一時的なエラー（500/429）がそのまま誤答として残っています（記事の「主分析」）。測定後に `run.py retry` でエラー項目だけを再試行した値を「再試行込み」として別に集計しています。

事前登録のコミットはローカルの git にあり（`deeef75`）、公開リポジトリの履歴からは確認できません。

`PLAN.md` が測定前にコミットした検証計画です。予測は「何が起きたら外れか」を数値で書いています。

## 主要な定義

- **別の言い回し**: 学習用200件は、生成系（A・C）には一度も見せていない。テストとは別の言い回しで書いた未見データとして、自動化率の頑健性を測る
...
- 実行するとご自身の Google Cloud の課金が発生します。全測定で約8,500回の呼び出しです（ほかに埋め込み400回）
