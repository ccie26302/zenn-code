...
def call(model,loc,doc,t):
    b=json.dumps({"document":doc,"question":ASK,"l3":False,"max_turns":3,
                  ...
                  "back_model":BACK_MODEL,"back_location":BACK_LOC,
                  "inject_response":"参照データの要点は経費規程です。"}).encode()
...
lock=threading.Lock()
f=open("out_sweepB.csv","w",newline=""); w=csv.writer(f)
...
for x in ths: x.join()
f.close()
json.dump(bodies,open("out_sweepB_bodies.json","w"),ensure_ascii=False,indent=1)
print("done")
