...
def call(model,loc,doc,t):
    b=json.dumps({"document":doc,"question":C["question"],"l3":False,"max_turns":3,
...
for m,loc in MODELS.items():
    ...
    for b in C["benigns"]:
        for s in range(B_REPS): trials.append((m,loc,b["id"],b["document"],"benign",s))
random.seed(20260910); random.shuffle(trials)
print("総試行:",len(trials))

t=tok(); t0=time.time(); n=0
lock=threading.Lock()
f=open("out_sweep.csv","w",newline=""); w=csv.writer(f)
w.writerow(["model","location","id","kind","sample","actions","turns_used","ii_tool","iii_hop",
...
for x in ths: x.join()
f.close()
json.dump(bodies,open("out_sweep_bodies.json","w"),ensure_ascii=False,indent=1)
print("done")
